...
- **可配置浏览器**：支持 Chrome、Edge 与 Firefox，自动探测安装路径
- **自动清理旧进程**：启动前终止残留的 dsh 进程，释放被占用的端口
- **关窗即停**：关闭启动器窗口即停止服务，无需手动结束进程
- **依赖自检**：未检测到 Node.js / dsh 时会给出清晰的安装提示；指定浏览器缺失时自动回退到其他受支持浏览器，全部不可用时提示手动访问
...
3. **浏览器**：Chrome、Edge 或 Firefox（任选其一）
...
| `$ProxyPort` | `7897` | 代理端口 |
| `$Browser` | `chrome` | 打开 UI 的浏览器：`chrome`、`edge` 或 `firefox` |
